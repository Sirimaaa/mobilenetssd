...
lineaccesstoken = '7Tqbc+gGFFUaXGn2TUsnEzwDZDTJLOJ2uQzl6YW8RBucP6cgWm85b/0oRAuc0hwNJMlInaoGQreK3u9Lmwvn2dHnoWF5QnBwWshNIcZcM8VFqrSp+Mzk3yWoyy8+PxZWXpEt9BeFeDE+hhQXUO4IzAdB04t89/1O/w1cDnyilFU='
...
def event_handle(event,json_line):
    ...
    if msgType == "text":
        ...
        if (msg == "สวัสดี") :
            replyObj = TextSendMessage(text="ดีค่า")
            line_bot_api.reply_message(rtoken, replyObj)
        elif (msg == "กินข้าวไหม") :
            replyObj = TextSendMessage(text="ไม่กิน")
            line_bot_api.reply_message(rtoken, replyObj)
        elif (msg == "ไปเที่ยวไหม") :
            replyObj = TextSendMessage(text="ไปด้วย")
            line_bot_api.reply_message(rtoken, replyObj)
        else :
            headers = request.headers 
            json_headers = ({k:v for k, v in 
            headers.items()}) 
            json_headers.update({'Host':'bots.dialogflow
            .com'}) 
            url = "https://dialogflow.cloud.google.com/v1/integrations/line/webhook/71ab6534-b88a-429c-ab9e-216b20bd53c1" 
